...
function isMissingOptionalCredentialError(error: unknown): boolean {
	if (!error || typeof error !== 'object') return false;
	const message = 'message' in error && typeof error.message === 'string' ? error.message : '';

	return /does not require credentials|credentials?.*not.*set|credential.*not.*found|could not find credentials/i.test(
		message,
	);
}
					...
					options: [
						{
							...
							action: 'Detect playable audio sources',
						},
						{
							name: 'Extract',
							value: 'extract',
							description: 'Return metadata and extracted data for a URL',
							action: 'Return metadata and extracted data for a URL',
						},
						{
							name: 'Insights',
							value: 'insights',
							description: 'Get performance and technology insights',
							action: 'Get performance and technology insights',
						},
						{
							name: 'Logo',
							value: 'logo',
							description: 'Return logo metadata, including logo.palette',
							action: 'Return logo metadata including logo palette',
						},
						{
							name: 'Markdown',
							value: 'markdown',
							description: 'Return page content in Markdown',
							action: 'Return page content in markdown',
						},
						{
							name: 'PDF',
							value: 'pdf',
						...
						},
						{
							...
							action: 'Return page content as plain text',
						},
						{
							name: 'Video',
							value: 'video',
							description: 'Detect playable video sources',
							action: 'Detect playable video sources',
						},
					],
					default: 'extract',
				},
				...
				},
				{
					...
					options: [
						...
						},
						{
							...
							description: 'Force a preferred color scheme when rendering the page',
						},
						{
		...
		for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
			try {
				...
				let credentials:
					...
					| undefined;
				try {
					credentials = (await this.getCredentials('microlinkApi')) as
						| { apiKey?: string; baseUrl?: string }
						| null
						| undefined;
				} catch (error) {
				...
				if (operation === 'audio') paramBag.audio = true;
				if (operation === 'video') paramBag.video = true;
				if (operation === 'insights') paramBag.insights = true;
				// Microlink exposes palette data under the logo value (logo.palette).
				if (operation === 'logo') paramBag.palette = true;
				...
				const legacyOverlayObject = isPlainObject(legacyScreenshotOverlay)
					? (legacyScreenshotOverlay as Record<string, unknown>)
					: undefined;
				const screenshotOverlayBackground =
					options.screenshotOverlayBackground ?? legacyOverlayObject?.background;
				const screenshotOverlayBrowser =
					options.screenshotOverlayBrowser ??
